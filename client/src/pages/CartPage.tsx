...
export default function CartPage() {
  ...
  return (
    <div className="space-y-2 sm:space-y-4">
      ...
      {items.length === 0 ? (
        <div className="bg-white border border-slate-100 rounded-xl p-2 sm:p-4 text-xs sm:text-sm">
        ...
        <>
          <div className="bg-white border border-slate-100 rounded-xl p-2 sm:p-4 text-xs sm:text-sm space-y-2 sm:space-y-2">
            {items.map((i) => (
              <div key={`${i.menuItem._id}:${i.sauce || ''}`} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-50 last:border-0 pb-2 last:pb-0 sm:border-0 sm:pb-0">
                <div className="flex-1">
                  ...
                  {String(i.menuItem.category || '').toLowerCase() === 'wings' && (
                    <div className="mt-1 text-[10px] sm:text-xs text-slate-600 flex items-center gap-2">
                      <span>Sauce:</span>
                      <select
                  ...
                  <div className="text-[10px] sm:text-xs text-slate-600 mt-0.5">₦{(i.menuItem.price * i.quantity).toLocaleString()}</div>
                </div>
                <div className="flex items-center gap-2 self-end sm:self-auto">
                  <button
                    ...
                    className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-xs sm:text-sm font-bold"
                  >
                    -
                  </button>
                  ...
                  <button
                    ...
                    className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-xs sm:text-sm font-bold ml-1"
                  >
                    ×
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 gap-2 mt-2">
            <button
              ...
              className="w-full px-4 py-2.5 sm:py-2 rounded-full bg-ration-green-hover text-white border border-slate-300 text-sm sm:text-sm disabled:opacity-60"
