    ...
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col animate-fade-in">
      ...
      <div className="p-2 sm:p-4 flex flex-col gap-1 sm:gap-2 flex-1">
        <div className="flex justify-between items-start gap-1 sm:gap-2">
          <div>
            <h3 className="text-sm sm:text-base font-semibold text-slate-800 leading-tight">{item.name}</h3>
        ...
        <p className="text-[10px] sm:text-xs text-slate-600 line-clamp-2 leading-snug">{item.description}</p>
          ...
          {quantity === 0 ? (
            ...
            <div className="flex items-center justify-between gap-1 sm:gap-3">
              <div className="flex items-center gap-1 sm:gap-2">
                <button
                  onClick={handleMinus}
                  className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-xs sm:text-sm font-bold"
                >
                  -
                </button>
