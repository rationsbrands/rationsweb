    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center space-y-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">MFA Enabled</h1>
            <p className="text-slate-600 mt-2">Your account is now secured with two-factor authentication.</p>
          </div>
          
          {backupCodes.length > 0 && (
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-left">
              <div className="font-semibold text-sm mb-2">Backup Codes</div>
              <div className="text-xs text-slate-500 mb-3">Save these codes in a secure place. You can use them to log in if you lose access to your device.</div>
              <div className="grid grid-cols-2 gap-2 font-mono text-sm">
                {backupCodes.map((c, i) => (
                  <div key={i} className="bg-white border rounded px-2 py-1 text-center">{c}</div>
                ))}
              </div>
            </div>
          )}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full space-y-6">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Setup MFA</h1>
          <p className="text-slate-600 mt-2 text-sm">Scan the QR code with your authenticator app (Google Authenticator, Authy, etc.)</p>
        </div>
        ...
        {loading && !qrCode ? (
          <div className="flex flex-col items-center justify-center py-8 space-y-4">
            ...
            <div className="text-sm text-slate-500">Generating secret...</div>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="text-center">
              ...
              <div className="font-mono bg-slate-100 py-2 px-4 rounded text-sm select-all cursor-pointer hover:bg-slate-200 transition-colors" onClick={() => navigator.clipboard.writeText(secret)}>
                {secret}
              </div>
            </div>
