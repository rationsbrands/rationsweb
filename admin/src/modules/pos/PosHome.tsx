...
export default function PosHome() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      <div className="bg-white dark:bg-slate-900 border rounded-xl p-3">
        <div className="flex items-center gap-2 mb-4">
          ...
          <button className="text-sm px-4 py-2 min-h-[44px] rounded-lg border hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors" onClick={refresh}>Refresh</button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
          {filtered.map(p => (
            <button key={p._id} className="border rounded-xl px-3 py-3 text-left hover:border-slate-400 transition-colors flex flex-col justify-between min-h-[80px]" onClick={()=>addItem(p)}>
              ...
              <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">₦{p.price}</div>
            </button>
          ))}
        </div>
      </div>
      <div className="bg-white dark:bg-slate-900 border rounded-xl p-3">
          ...
          {items.map(i => (
            <div key={i.product._id} className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 border rounded-lg p-2">
              <div className="flex-1 min-w-[120px]">
                ...
                <div className="text-xs text-slate-600 dark:text-slate-300">₦{i.product.price}</div>
              </div>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex items-center gap-2">
            <button className="text-sm px-4 py-2 min-h-[44px] rounded-lg border hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors" onClick={clear}>Clear</button>
